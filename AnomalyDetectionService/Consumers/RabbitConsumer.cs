    ...
    : IMessageConsumer<T>
{
    ...
    private async Task HandleMessageAsync(BasicDeliverEventArgs ea, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            logger.LogInformation("[RabbitMqConsumer] Deserialized data successfully : {Data}",
    ...
    {
        logger.LogWarning(
            "[RabbitMqConsumer] Invalid message received. Nacking message with delivery tag: {DeliveryTag}",
            ea.DeliveryTag);
        await _channel.BasicNackAsync(ea.DeliveryTag, false, false, cancellationToken);
    }
}